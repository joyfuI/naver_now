...
schedule_modal_save_btn.addEventListener('click', (event) => {
  ...
  const week = [
    ...document.querySelectorAll('#interval_week option:checked'),
  ].map((option) => option.value);
  interval.value = `${parseInt(min)} ${parseInt(hour)} * * ${week.join(',')}`;
  if (!/^https?:\/\/now\.naver\.com\/((player\/)|(l\/))?\d+/u.test(url.value)) {
...
function make_item(data) {
  ...
  tmp = `<input class="form-control form-control-sm now-sch" type="checkbox" data-toggle="toggle" data-id="${
    data.id
  }" ${data.is_include ? 'checked' : ''}>`;
  tmp += `<span style="padding-left: 10px; padding-top: 10px;">${
    data.is_running ? '동작중' : '대기중'
  }</span>`;
