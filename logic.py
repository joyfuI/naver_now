...
class Logic(object):
    ...
    def scheduler_start(job_id):
        try:
            ...
            job = Job(
                package_name,
                f"{package_name}_{job_id}",
                scheduler_model.interval,
                Logic.scheduler_function,
                f"NAVER NOW 라이브 다운로드:{scheduler_model.title}",
                False,
                job_id,
            )
    ...
    def scheduler_stop(job_id):
        try:
            ...
            scheduler.remove_job(f"{package_name}_{job_id}")
        except Exception as e:
            logger.error("Exception:%s", e)
            logger.error(traceback.format_exc())
    ...
    def one_execute(job_id):
        try:
            if job_id is None:
                ...
                if scheduler.is_include(f"{package_name}_{job_id}"):
                    if scheduler.is_running(f"{package_name}_{job_id}"):
                        ret = "is_running"
                    else:
                        # scheduler.execute_job('%s_%s' % (package_name, job_id))
                        # 뜸들이지말고 바로 실행
                        logger.debug("execute_job:%s", f"{package_name}_{job_id}")
                        job = scheduler.sched.get_job(f"{package_name}_{job_id}")
